...
public class Pusher implements PusherEventEmitter {
	...
	private static final String POTOCOL = "6";
	...
	public PusherChannel mGlobalChannel = new PusherChannel(
			"pusher_global_channel");
	...
	private String getPath() {
		return "/app/" + mPusherKey + "?client=" + PUSHER_CLIENT + "&version="
				+ VERSION + "&potocol=" + POTOCOL;
	}
	...
	private void unsubscribeFromAllChannels() {
		...
		/*
		 * TODO: just mark the channels as unsubscribed in order to keep the
		 * bindings
		 */
	...
	private String authenticate(String channelName) {
		...
		try {
			...
			SecretKey key = new SecretKeySpec(mPusherSecret.getBytes(),
					PUSHER_AUTH_ALGORITHM);
